...
from analysis_common import ensure_output_dir, merge_signal_and_feature, ror_prr_from_counts, save_tables, two_by_two_counts
        ...
        rows.append(
            {
                ...
                "n_feature_positive_fall": fall_n,
                "fall_reporting_rate": (fall_n / exposed_n) if exposed_n else None,
                **metrics,
            }
        )
...
def build_feature_analysis(signal_root: str | Path | None = None, output_dir: str | Path | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for analysis_name, exposure_col in [
        ...
        result_frames.append(_build_feature_rows(subset, analysis_name))
        qc_rows.append(
            {
                "analysis": analysis_name,
                "n_zolpidem_exposed": int(len(subset)),
                "n_fall": int(subset[OUTCOME_COL].fillna(False).astype(bool).sum()),
    ...
    result_df = result_df.sort_values(["analysis", "ror"], ascending=[True, False], na_position="last")
